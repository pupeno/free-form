...
(defn- bind-field [values on-change node]
  (if (not (field? node))
    node
    (let [[attributes _ ks] (extract-attributes node :free-form/input)]
...
(defn- expand-bootstrap-horizontal-fields [node]
  (if (bootstrap-horizontal-fields? node)
    (let [{:keys [type ks label placeholder]} (key->keys (second node))
          id (clojure.string/join "-" (map name ks))]
      [:div.form-group {:free-form/error-class {:ks ks :error "has-error"}}
       [:label.col-sm-2.control-label {:for id} label]
       [:div.col-sm-10 [:input.form-control {:free-form/input {:ks ks}
                                             :type            type
                                             :id              id
                                             :placeholder     placeholder}]
        [:div.text-danger {:free-form/error-message {:ks ks}} [:p]]]])
    node))
